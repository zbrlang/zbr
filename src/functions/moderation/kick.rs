...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let uids_str = match args.get(0) {
        ...
        _ => return FnOutput::error("kick", crate::error_messages::required(1, "userIDs")),
    };
    ...
    {
        Ok(ids) => ids,
        Err(_) => return FnOutput::error("kick", crate::error_messages::expected_snowflake(1, "userIDs", &uids_str)),
    };

    if user_ids.is_empty() {
        return FnOutput::error("kick", crate::error_messages::required(1, "userIDs"));
    }
    ...
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("kick", crate::error_messages::not_in_guild()),
    };
