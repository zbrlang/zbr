...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let event_id_str = match args.get(0) {
        ...
        _ => return FnOutput::error("editEvent", crate::error_messages::required(1, "eventID")),
    };
    let eid: u64 = match event_id_str.parse() {
        Ok(id) => id,
        Err(_) => {
            return FnOutput::error("editEvent", crate::error_messages::expected_snowflake(1, "eventID", &event_id_str))
        }
    };
    ...
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("editEvent", crate::error_messages::not_in_guild()),
    };
    ...
    let result = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async move {
            ...
            GuildId::new(gid)
                ...
                .await
                .map_err(|e| crate::error_messages::action_failed_reason("edit event", &e.to_string()))
        })
    });
