...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let name = match args.get(0) {
        ...
        _ => return FnOutput::error("createEvent", crate::error_messages::required(1, "name")),
    };
    let channel_id_str = match args.get(1) {
        Some(s) if !s.is_empty() => s.clone(),
        _ => return FnOutput::error("createEvent", crate::error_messages::required(2, "channel ID")),
    };
    let start_time_str = match args.get(2) {
        Some(s) if !s.is_empty() => s.clone(),
        _ => return FnOutput::error("createEvent", crate::error_messages::required(3, "startTime")),
    };
    ...
    let cid: u64 = match channel_id_str.parse() {
        ...
        Err(_) => {
            return FnOutput::error(
                "createEvent",
                crate::error_messages::expected_snowflake(2, "channel ID", &channel_id_str),
            )
        }
    };
    let start_unix: i64 = match start_time_str.parse() {
        Ok(n) => n,
        Err(_) => return FnOutput::error("createEvent", crate::error_messages::expected_integer(3, "startTime", &start_time_str)),
    };
    ...
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("createEvent", crate::error_messages::not_in_guild()),
    };
