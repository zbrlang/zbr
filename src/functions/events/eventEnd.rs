...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let eid_str = match args.get(0) {
        ...
        _ => return FnOutput::error("eventEnd", crate::error_messages::required(1, "eventID")),
    };
    let eid: u64 = match eid_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("eventEnd", crate::error_messages::expected_snowflake(1, "eventID", &eid_str)),
    };
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("eventEnd", crate::error_messages::not_in_guild()),
    };
