...
pub fn run(args: Vec<String>, _ctx: &DiscordContext) -> FnOutput {
    ...
    if secs_str.is_empty() {
        return FnOutput::error("duration", "seconds is required");
    }

    let secs: i64 = match secs_str.parse() {
        Ok(s) => s,
        Err(_) => return FnOutput::error("duration", "seconds must be a number"),
    };

    if secs < 0 {
        return FnOutput::error("duration", "seconds must be non-negative");
    }
