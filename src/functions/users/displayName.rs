...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let uid: u64 = match user_id_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("displayName", crate::error_messages::expected_snowflake(1, "userID", &user_id_str)),
    };
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("displayName", crate::error_messages::not_in_guild()),
    };
    ...
    match result {
        ...
        Err(_) => FnOutput::error("displayName", crate::error_messages::not_found("user", &user_id_str)),
    }
}
