...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let uid: u64 = match user_id_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("userServerAvatar", "invalid userID"),
    };

    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("userServerAvatar", "not in a guild"),
    };
    ...
    match result {
        ...
        Err(_) => FnOutput::error("userServerAvatar", "user not found"),
    }
}
