use super::helpers::{
    parse_index, validate_bool, validate_channel_same_guild, validate_embed_sendable,
    validate_snowflake,
};
...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    if let Err(e) = validate_channel_same_guild(channel_id, ctx, http.clone(), "sendEmbed") {
        return e;
    }
    ...
    if let Err(e) = validate_embed_sendable(&embed_data, "sendEmbed", index) {
        return e;
    }
