...
pub fn run(_args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("randomUser", "no HTTP client available"),
    };
    let guild_id: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("randomUser", crate::error_messages::not_in_guild()),
    };
    ...
    match result {
        ...
        Err(e) => FnOutput::error("randomUser", e),
    }
}
