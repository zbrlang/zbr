...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("voiceMembers", "not in a guild"),
    };
    ...
    let channel_id: u64 = match args.get(0) {
        Some(s) if !s.is_empty() => match s.parse() {
            Ok(id) => id,
            Err(_) => {
                return FnOutput::error("voiceMembers", format!("invalid channel ID: '{}'", s))
            }
        },
        _ => {
            ...
            let author_uid: u64 = match ctx.author_id.parse() {
                Ok(id) => id,
                Err(_) => return FnOutput::error("voiceMembers", "invalid author ID"),
            };
