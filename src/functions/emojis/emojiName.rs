...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let emoji_id = match validate_snowflake(&id_str, "emojiName", "emoji ID") {
        Ok(id) => id, Err(e) => return e,
    };

    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("emojiName", "no HTTP client available"),
    };

    let guild_id: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("emojiName", crate::error_messages::not_in_guild()),
    };
    ...
    match result {
        ...
        Err(e) => FnOutput::error("emojiName", e),
    }
}
