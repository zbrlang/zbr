...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let tid_str = match args.get(0) {
        ...
        _ => return FnOutput::error("threadUnarchive", crate::error_messages::required(1, "threadID")),
    };
    let tid: u64 = match tid_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("threadUnarchive", crate::error_messages::expected_snowflake(1, "thread ID", &tid_str)),
    };
