...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("lowestRole", crate::error_messages::not_in_guild()),
    };

    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("lowestRole", "no HTTP client available"),
    };
    ...
    let result = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async move {
            ...
            let lowest = member.roles.iter()
                .filter_map(|rid| roles.get(rid))
                .filter(|r| r.id.get() != gid) // Exclude @everyone
                .min_by_key(|r| r.position);
            Ok::<String, String>(lowest.map(|r| r.id.to_string()).unwrap_or_default())
        })
    });

    match result {
        Ok(id) => FnOutput::Text(id),
        Err(e) => FnOutput::error("lowestRole", e),
    }
}
