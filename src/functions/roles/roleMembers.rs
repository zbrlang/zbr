...
/// ZroleMembers{roleID?}
/// Returns a space-separated list of member IDs that have the given role.
/// Defaults to the author's highest role when no roleID is given.
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("roleMembers", crate::error_messages::not_in_guild()),
    };
    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("roleMembers", "no HTTP client available"),
    };
    ...
    let result = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async move {
            ...
            loop {
                ...
                for m in &batch {
                    if m.roles.contains(&RoleId::new(rid)) {
                        members.push(m.user.id.to_string());
                    }
                }
                if batch.len() < 1000 {
                    break;
            ...
            Ok::<Vec<String>, String>(members)
        })
    });

    match result {
        Ok(ids) => FnOutput::Text(ids.join(" ")),
        Err(e) => FnOutput::error("roleMembers", e),
    }
}
