...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let role_id_str = if args.get(0).map(|s| s.is_empty()).unwrap_or(true) {
        ...
        match member {
            ...
            Err(_) => return FnOutput::error("roleName", crate::error_messages::action_failed("get author's top role")),
        }
    } else {
        args[0].clone()
    };
    ...
    let rid: u64 = match role_id_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("roleName", "invalid role ID"),
    };

    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("roleName", crate::error_messages::not_in_guild()),
    };

    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("roleName", "no HTTP client available"),
    };
    ...
    match result {
        Ok(name) => FnOutput::Text(name),
        Err(e) => FnOutput::error("roleName", e),
    }
}
