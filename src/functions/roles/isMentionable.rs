...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let role_id_str = if args.get(0).map(|s| s.is_empty()).unwrap_or(true) {
        ...
        match member {
            ...
            Err(_) => return FnOutput::error("isMentionable", "could not get author's top role"),
        }
    } else {
        args[0].clone()
    };
    ...
    let rid: u64 = match role_id_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("isMentionable", crate::error_messages::expected_snowflake(1, "role ID", &role_id_str)),
    };

    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("isMentionable", crate::error_messages::not_in_guild()),
    };

    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("isMentionable", "no HTTP client available"),
    };
    ...
    match result {
        ...
        Err(e) => FnOutput::error("isMentionable", e),
    }
}
