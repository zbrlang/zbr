...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let cid: u64 = match cid_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("lastMessageID", "invalid channel ID"),
    };

    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("lastMessageID", "no HTTP client available"),
    };
    ...
    match result {
        ...
        Err(_) => FnOutput::error("lastMessageID", "channel not found"),
    }
}
