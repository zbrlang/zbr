...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let cid: u64 = match cid_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("lastPinTimestamp", "invalid channel ID"),
    };

    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("lastPinTimestamp", "no HTTP client available"),
    };
    ...
    let ts_opt = match result {
        ...
        Err(_) => return FnOutput::error("lastPinTimestamp", "channel not found"),
    };
