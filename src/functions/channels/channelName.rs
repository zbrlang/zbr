...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let cid: u64 = match cid_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("channelName", crate::error_messages::expected_snowflake(1, "channel ID", &cid_str)),
    };

    let http = match &ctx.http {
        Some(h) => h.clone(),
        None => return FnOutput::error("channelName", crate::error_messages::requires_set_first("HTTP client")),
    };
    ...
    match result {
        ...
        Err(_) => FnOutput::error("channelName", crate::error_messages::not_found("channel", &cid_str)),
    }
}
