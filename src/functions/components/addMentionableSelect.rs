...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let menu_id = match args.get(0) {
        ...
        _ => return FnOutput::error("addMentionableSelect", "menuID is required"),
    };
    let min_values: u8 = match args.get(1) {
        Some(s) if !s.is_empty() => match s.parse() {
            Ok(n) => n,
            Err(_) => return FnOutput::error("addMentionableSelect", format!("invalid min: '{}'", s)),
        },
        _ => 1,
    };
    let max_values: u8 = match args.get(2) {
        Some(s) if !s.is_empty() => match s.parse() {
            Ok(n) => n,
            Err(_) => return FnOutput::error("addMentionableSelect", format!("invalid max: '{}'", s)),
        },
        _ => 1,
    };
