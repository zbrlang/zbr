...
/// ZgetMentionableSelectUserIDs{separator;limit?} — returns all selected user/role IDs joined by separator.
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let separator = args.get(0).filter(|s| !s.is_empty()).cloned().unwrap_or_else(|| ", ".to_string());
