...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let guild_id = match guild_id_str.parse::<u64>() {
        ...
        Err(_) => return FnOutput::error("serverName", "guild not found"),
    };

    let http = match ctx.http.as_ref() {
        Some(h) => h.clone(),
        None => return FnOutput::error("serverName", "no HTTP client available"),
    };
    tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async move {
            match http.get_guild(guild_id).await {
                Ok(guild) => FnOutput::Text(guild.name.clone()),
                Err(_) => FnOutput::error("serverName", "guild not found"),
            }
        })
    })
}
