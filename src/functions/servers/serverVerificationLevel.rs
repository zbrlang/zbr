...
pub fn run(_args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    if ctx.guild_id.is_empty() {
        return FnOutput::error("serverVerificationLevel", "not in a guild");
    }

    let guild_id = match ctx.guild_id.parse::<u64>() {
        Ok(id) => GuildId::new(id),
        Err(_) => return FnOutput::error("serverVerificationLevel", "guild not found"),
    };

    let http = match ctx.http.as_ref() {
        Some(h) => h.clone(),
        None => return FnOutput::error("serverVerificationLevel", "no HTTP client available"),
    };
    tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async move {
            match http.get_guild(guild_id).await {
                ...
                Err(_) => FnOutput::error("serverVerificationLevel", "guild not found"),
            }
        })
    })
}
