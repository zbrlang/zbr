...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let guild_id = match guild_id_str.parse::<u64>() {
        ...
        Err(_) => return FnOutput::error("serverBanner", crate::error_messages::not_found("guild", &guild_id_str)),
    };

    let http = match ctx.http.as_ref() {
        Some(h) => h.clone(),
        None => return FnOutput::error("serverBanner", "no HTTP client available"),
    };
    tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async move {
            match http.get_guild(guild_id).await {
                ...
                Err(_) => FnOutput::error("serverBanner", crate::error_messages::not_found("guild", &guild_id_str)),
            }
        })
    })
}
