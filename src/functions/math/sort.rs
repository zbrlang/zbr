...
pub fn run(args: Vec<String>, _ctx: &DiscordContext) -> FnOutput {
    if args.len() < 5 {
        return FnOutput::error("sort", "requires at least 2 numbers, a direction, a return amount, and a separator");
    }
    ...
    let ascending = match direction.as_str() {
        "asc"  => true,
        "desc" => false,
        other  => return FnOutput::error("sort", format!("invalid direction: '{}' (expected 'asc' or 'desc')", other)),
    };

    // Parse return amount
    let ret_n = match parse_i64(&ret_amount, "sort", "return amount") {
        Ok(v) => v, Err(e) => return e,
    };
    if ret_n < -1 || ret_n == 0 {
        return FnOutput::error("sort", format!("return amount must be -1 (all) or a positive integer, got {}", ret_n));
    }

    // Parse numbers
    let mut numbers: Vec<f64> = Vec::with_capacity(number_args.len());
    for (i, arg) in number_args.iter().enumerate() {
        let n = match parse_f64(arg, "sort", &format!("argument {}", i + 1)) {
            Ok(v) => v, Err(e) => return e,
        };
        numbers.push(n);
    }
