...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let sid_str = match args.get(0) {
        ...
        _ => return FnOutput::error("stickerDescription", crate::error_messages::required(1, "stickerID")),
    };
    let sid: u64 = match sid_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("stickerDescription", crate::error_messages::expected_snowflake(1, "stickerID", &sid_str)),
    };
    let gid: u64 = match ctx.guild_id.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("stickerDescription", crate::error_messages::not_in_guild()),
    };
