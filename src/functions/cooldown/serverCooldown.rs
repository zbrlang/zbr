...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    ...
    let result = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async {
            crate::db::try_acquire_server_cooldown(&db, &bot_id, &guild_id, &command, duration_secs).await
        })
    });
    ...
    match result {
        ...
        Err(e) => FnOutput::error("serverCooldown", crate::error_messages::action_failed_reason("acquire server cooldown", &e.to_string())),
    }
}
