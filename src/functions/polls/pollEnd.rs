...
pub fn run(args: Vec<String>, ctx: &DiscordContext) -> FnOutput {
    let mid_str = match args.get(0) {
        ...
        _ => return FnOutput::error("pollEnd", crate::error_messages::required(1, "messageID")),
    };
    let mid: u64 = match mid_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("pollEnd", crate::error_messages::expected_snowflake(1, "messageID", &mid_str)),
    };
    let cid_str = args.get(1).cloned().unwrap_or_else(|| ctx.channel_id.clone());
    let cid: u64 = match cid_str.parse() {
        Ok(id) => id,
        Err(_) => return FnOutput::error("pollEnd", crate::error_messages::expected_snowflake(2, "channelID", &cid_str)),
    };
