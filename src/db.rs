...
pub async fn connect() -> SqlitePool {
    ...
    ").execute(&pool).await.expect("Failed to create user_vars table");
    ...
    ").execute(&pool).await.expect("Failed to create server_vars table");
    ...
    ").execute(&pool).await.expect("Failed to create channel_vars table");
    ...
    ").execute(&pool).await.expect("Failed to create global_vars table");
    ...
    ").execute(&pool).await.expect("Failed to create user_cooldowns table");
    ...
    ").execute(&pool).await.expect("Failed to create server_cooldowns table");
    ...
    ").execute(&pool).await.expect("Failed to create global_cooldowns table");

    println!("Database connected");
    pool
}

pub async fn get_user_var(pool: &SqlitePool, bot_id: &str, guild_id: &str, user_id: &str, name: &str) -> String {
    sqlx::query_scalar::<_, String>(
        "SELECT value FROM user_vars WHERE bot_id=? AND guild_id=? AND user_id=? AND name=?"
    )
    .bind(bot_id).bind(guild_id).bind(user_id).bind(name)
    .fetch_optional(pool).await.unwrap_or(None).unwrap_or_default()
}

pub async fn set_user_var(pool: &SqlitePool, bot_id: &str, guild_id: &str, user_id: &str, name: &str, value: &str) {
    sqlx::query(
    ...
    .bind(bot_id).bind(guild_id).bind(user_id).bind(name).bind(value)
...
pub async fn reset_user_var(pool: &SqlitePool, bot_id: &str, guild_id: &str, name: &str) {
    sqlx::query(
        "DELETE FROM user_vars WHERE bot_id=? AND guild_id=? AND name=?"
    )
    .bind(bot_id).bind(guild_id).bind(name)
    .execute(pool).await.ok();
}
...
pub async fn set_server_var(pool: &SqlitePool, bot_id: &str, guild_id: &str, name: &str, value: &str) {
    sqlx::query(
        "INSERT INTO server_vars (bot_id, guild_id, name, value)
         VALUES (?, ?, ?, ?)
         ON CONFLICT(bot_id, guild_id, name) DO UPDATE SET value=excluded.value"
    )
    .bind(bot_id).bind(guild_id).bind(name).bind(value)
    .execute(pool).await.ok();
}
...
pub async fn reset_server_var(pool: &SqlitePool, bot_id: &str, name: &str) {
    sqlx::query(
        "DELETE FROM server_vars WHERE bot_id=? AND name=?"
    )
    .bind(bot_id).bind(name)
    .execute(pool).await.ok();
}

pub async fn get_channel_var(pool: &SqlitePool, bot_id: &str, channel_id: &str, name: &str) -> String {
    sqlx::query_scalar::<_, String>(
        "SELECT value FROM channel_vars WHERE bot_id=? AND channel_id=? AND name=?"
    )
    .bind(bot_id).bind(channel_id).bind(name)
    .fetch_optional(pool).await.unwrap_or(None).unwrap_or_default()
}

pub async fn set_channel_var(pool: &SqlitePool, bot_id: &str, channel_id: &str, name: &str, value: &str) {
    sqlx::query(
    ...
    .bind(bot_id).bind(channel_id).bind(name).bind(value)
    .execute(pool).await.ok();
}
...
pub async fn reset_channel_var(pool: &SqlitePool, bot_id: &str, name: &str) {
    sqlx::query(
        "DELETE FROM channel_vars WHERE bot_id=? AND name=?"
    )
    .bind(bot_id).bind(name)
    .execute(pool).await.ok();
}
...
pub async fn get_global_var(pool: &SqlitePool, bot_id: &str, name: &str) -> String {
    sqlx::query_scalar::<_, String>(
        "SELECT value FROM global_vars WHERE bot_id=? AND name=?"
    )
    .bind(bot_id).bind(name)
    .fetch_optional(pool).await.unwrap_or(None).unwrap_or_default()
}
...
pub async fn set_global_var(pool: &SqlitePool, bot_id: &str, name: &str, value: &str) {
    ...
    .bind(bot_id).bind(name).bind(value)
    .execute(pool).await.ok();
}
...
pub async fn list_global_vars(pool: &SqlitePool, bot_id: &str) -> Vec<String> {
    sqlx::query_scalar::<_, String>(
        "SELECT name FROM global_vars WHERE bot_id=? ORDER BY name"
    )
    .bind(bot_id)
...
pub async fn global_var_exists(pool: &SqlitePool, bot_id: &str, name: &str) -> bool {
    sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM global_vars WHERE bot_id=? AND name=?"
    )
    .bind(bot_id).bind(name)
...
pub async fn get_user_cooldown(pool: &SqlitePool, bot_id: &str, guild_id: &str, user_id: &str, command: &str) -> i64 {
...
pub async fn set_user_cooldown(pool: &SqlitePool, bot_id: &str, guild_id: &str, user_id: &str, command: &str, duration_secs: i64) {
...
pub async fn get_server_cooldown(pool: &SqlitePool, bot_id: &str, guild_id: &str, command: &str) -> i64 {
    let now = chrono::Utc::now().timestamp();
    let expires: Option<i64> = sqlx::query_scalar(
        "SELECT expires_at FROM server_cooldowns WHERE bot_id=? AND guild_id=? AND command=?"
    )
    .bind(bot_id).bind(guild_id).bind(command)
    .fetch_optional(pool).await.unwrap_or(None);
    match expires {
        Some(e) if e > now => e - now,
        _ => 0,
...
pub async fn set_server_cooldown(pool: &SqlitePool, bot_id: &str, guild_id: &str, command: &str, duration_secs: i64) {
    let expires_at = chrono::Utc::now().timestamp() + duration_secs;
    sqlx::query(
    ...
    .bind(bot_id).bind(guild_id).bind(command).bind(expires_at)
    .execute(pool).await.ok();
}

/// Returns remaining seconds on a global cooldown, or 0 if not active.
pub async fn get_global_cooldown(pool: &SqlitePool, bot_id: &str, user_id: &str, command: &str) -> i64 {
    let now = chrono::Utc::now().timestamp();
    let expires: Option<i64> = sqlx::query_scalar(
        "SELECT expires_at FROM global_cooldowns WHERE bot_id=? AND user_id=? AND command=?"
    )
    .bind(bot_id).bind(user_id).bind(command)
    .fetch_optional(pool).await.unwrap_or(None);
    match expires {
        Some(e) if e > now => e - now,
        _ => 0,
...
pub async fn set_global_cooldown(pool: &SqlitePool, bot_id: &str, user_id: &str, command: &str, duration_secs: i64) {
    let expires_at = chrono::Utc::now().timestamp() + duration_secs;
    sqlx::query(
    ...
    .bind(bot_id).bind(user_id).bind(command).bind(expires_at)
    .execute(pool).await.ok();
}
