...
function listCommands() {
  if (!fs.existsSync("commands")) {
    console.log(
      "No commands/ directory found. Create one with `zbr init` or add .zbr files to commands/.",
    );
    return;
  }
  ...
  const files = fs
    ...
    .filter((file) => file.endsWith(".zbr"));
  if (files.length === 0) {
    console.log(
      "No .zbr command files found in commands/. Create one with `zbr new <type>`.",
    );
    return;
  }
...
function getBinaryPath() {
  ...
  const binaryMap = {
    "linux-x64": "@zbrlang/zbr-linux-x64",
    "linux-arm64": "@zbrlang/zbr-linux-arm64",
    "darwin-x64": "@zbrlang/zbr-darwin-x64",
    "darwin-arm64": "@zbrlang/zbr-darwin-arm64",
    "win32-x64": "@zbrlang/zbr-windows-x64",
    "win32-arm64": "@zbrlang/zbr-windows-arm64",
  };

  const key = `${platform}-${arch}`;
  const pkgName = binaryMap[key];
  ...
  try {
    ...
    const binaryName =
      pkgName.replace("@zbrlang/", "").replace("-", "_") +
      (platform === "win32" ? ".exe" : "");
    const binaryMapFiles = {
...
function update() {
  console.log("Updating ZBR runtime engine...");
  try {
    execSync("npm i -g @zbrlang/zbr@latest", { stdio: "inherit" });
    console.log("Update successful!");
  } catch (err) {
    console.error(
      "Update failed. Please try running `npm i -g @zbrlang/zbr@latest` manually.",
    );
    process.exit(1);
  }
}
